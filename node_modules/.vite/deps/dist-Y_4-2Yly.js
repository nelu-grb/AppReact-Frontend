...
function createAccountEntityFromAccountInfo(accountInfo, cloudGraphHostName, msGraphHost) {
	...
	if (tenantProfiles.length === 0 && accountInfo.tenantId && accountInfo.localAccountId) tenantProfiles.push(buildTenantProfile(accountInfo.homeAccountId, accountInfo.localAccountId, accountInfo.tenantId, accountInfo.nativeAccountId, accountInfo.idTokenClaims));
	else if (accountInfo.nativeAccountId) {
		const matchingProfile = tenantProfiles.find((tp) => tp.tenantId === accountInfo.tenantId);
		if (matchingProfile && !matchingProfile.nativeAccountId) matchingProfile.nativeAccountId = accountInfo.nativeAccountId;
	}
	return {
...
function getAliasesFromMetadata(logger, correlationId, authorityHost, cloudDiscoveryMetadata, source) {
	...
	if (authorityHost && cloudDiscoveryMetadata) {
		...
		} else logger.trace(`14avvj ${source}`, correlationId);
	}
	return null;
}
/**
	...
	if (atEntity.tokenType?.toLowerCase() !== AuthenticationScheme$1.BEARER.toLowerCase()) {
		...
		switch (normalizedTokenType) {
			case AuthenticationScheme$1.POP:
				...
				break;
			case "dpop":
				if (!keyId) throw createClientAuthError(keyIdMissing, correlationId);
...
var Authority = class Authority {
	...
	*/
	...
	updateCloudDiscoveryMetadataFromLocalSources(metadataEntity) {
		...
		this.logger.verbosePii(`1fy7uz ${this.authorityOptions.knownAuthorities || "N/A"}`, this.correlationId);
		this.logger.verbosePii(`08zabj ${this.authorityOptions.authorityMetadata || "N/A"}`, this.correlationId);
		this.logger.verbosePii(`1o1kv3 ${metadataEntity.canonical_authority || "N/A"}`, this.correlationId);
	...
	*/
	async getCloudDiscoveryMetadataFromNetwork() {
		...
		try {
			...
			if (error instanceof AuthError) this.logger.error(`0vwhc7 ${error.errorCode} ${error.errorMessage}`, this.correlationId);
			else {
...
var ScopeSet = class ScopeSet {
	...
	*/
	static createSearchScopes(inputScopeString, correlationId) {
		const scopesToUse = inputScopeString && inputScopeString.length > 0 ? inputScopeString : [...OIDC_DEFAULT_SCOPES$1];
...
function addClaims(parameters, correlationId, claims, clientCapabilities, skipBrokerClaims, claimsToMerge) {
	const mergedClaims = buildMergedClaims$1(claims, skipBrokerClaims && parameters.has("brk_client_id") ? void 0 : clientCapabilities, correlationId, claimsToMerge);
...
var DEFAULT_ID_TOKEN_CLAIMS = {
	...
	[ClaimsRequestKeys.TENANT_REGION_SUB_SCOPE]: { essential: false }
};
/**
...
function parseClaims(claims, correlationId = "") {
	...
	if (!isPlainObject$1(parsed)) throw createClientConfigurationError(invalidClaims, correlationId);
	return parsed;
}
/**
...
function deepMergeClaims(baseClaims, claimsToMerge) {
	...
	for (const [key, mergeInValue] of Object.entries(claimsToMerge)) {
		...
		if (isPlainObject$1(baseValue) && isPlainObject$1(mergeInValue)) merged[key] = deepMergeClaims(baseValue, mergeInValue);
...
//#endregion
//#region node_modules/@azure/msal-common/dist-browser/crypto/ITokenBindingKeyManager.mjs
/*! @azure/msal-common v16.14.0 2026-08-28 */
/**
...
var CacheManager = class {
	...
	*/
	credentialMatchesFilter(entity, filter, correlationId) {
		...
		if (entity.credentialType === CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME) {
			if (!this.matchAccessTokenWithAuthScheme(entity, filter)) return false;
		}
	...
	*/
	removeAccessToken(key, correlationId) {
		...
		if (credential.credentialType.toLowerCase() === CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME.toLowerCase()) switch (credential.tokenType?.toLowerCase()) {
			case AuthenticationScheme$1.POP:
			case AuthenticationScheme$1.DPOP.toLowerCase(): {
				const kid = credential.keyId;
				if (kid) this.tokenBindingKeyManager.removeTokenBindingKey(kid, correlationId).catch(() => {
	...
	*/
	getAccessToken(account, request, tokenKeys, targetRealm) {
		...
		const accessTokenFilter = {
			...
			keyId: authScheme === AuthenticationScheme$1.SSH ? request.sshKid : void 0,
	...
	*/
	...
	matchAccessTokenWithAuthScheme(entity, filter) {
		...
		switch (normalizedFilterTokenType) {
			case "dpop":
			case AuthenticationScheme$1.SSH: return this.matchKeyBoundAccessToken(entity, filter);
			default: return true;
		}
	}
	matchKeyBoundAccessToken(entity, filter) {
		if (!filter.keyId) return true;
...
function buildClientConfiguration({ authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, tokenBindingKeyManager, clientCredentials, libraryInfo, telemetry, serverTelemetryManager, persistencePlugin, serializableCache }) {
	...
	return {
		...
		loggerOptions,
		storageInterface: storageImplementation || new DefaultStorageClass(userAuthOptions.clientId, DEFAULT_CRYPTO_IMPLEMENTATION, new Logger(loggerOptions, "@azure/msal-common", "16.14.0"), new StubPerformanceClient(), void 0, resolvedTokenBindingKeyManager),
...
//#endregion
//#region node_modules/@azure/msal-common/dist-browser/utils/ObjectUtils.mjs
/*! @azure/msal-common v16.14.0 2026-08-28 */
/**
...
var JoseHeader = class JoseHeader {
	constructor(options, correlationId) {
		if (typeof options.alg !== "string" || !options.alg) throw createJoseHeaderError(missingAlgError, correlationId);
	...
	*/
	static getShrHeader(shrHeaderOptions, correlationId) {
		if (!shrHeaderOptions.kid) throw createJoseHeaderError(missingKidError, correlationId);
		if (!shrHeaderOptions.alg) throw createJoseHeaderError(missingAlgError, correlationId);
	...
	*/
	static getDpopHeader(dpopHeaderOptions, correlationId) {
		if (!isPlainObject(dpopHeaderOptions.jwk)) throw createJoseHeaderError(missingJwkError, correlationId);
		if (!dpopHeaderOptions.alg) throw createJoseHeaderError(missingAlgError, correlationId);
		if (Object.keys(dpopHeaderOptions.jwk).length === 0) throw createJoseHeaderError(invalidJwkError, correlationId);
		return new JoseHeader({
...
var PopTokenGenerator = class {
	...
	*/
	async generateKid(request) {
		return {
			kid: await this.tokenBindingKeyManager.provisionTokenBindingKey({
	...
	*/
	async signPayload(payload, keyId, request, claims) {
		...
		const shrPayload = {
			...
			...claims,
			cnf: { jwk: publicKeyJwk }
		};
		return this.cryptoUtils.signTokenBindingJwt(shrHeader, shrPayload, keyId, request.correlationId);
	}
};
//#endregion
//#region node_modules/@azure/msal-common/dist-browser/crypto/DpopProofGenerator.mjs
/*! @azure/msal-common v16.14.0 2026-08-28 */
...
function normalizeHtm(htm, correlationId) {
	if (typeof htm !== "string" || !DPOP_HTM_REGEX.test(htm)) throw createClientConfigurationError(invalidDpopHtm, correlationId);
...
function normalizeHtu(url, correlationId) {
	...
	if (!/^https:\/\//i.test(url) || parsedUrl.protocol !== "https:" || parsedUrl.username || parsedUrl.password) throw createClientConfigurationError(invalidDpopHtu, correlationId);
	parsedUrl.search = "";
	parsedUrl.hash = "";
	return parsedUrl.href;
}
function validateDpopNonce(nonce, correlationId) {
	if (nonce !== void 0 && nonce.trim().length === 0) throw createClientConfigurationError(invalidDpopNonce, correlationId);
}
/**
...
var DpopProofGenerator = class {
	...
	*/
	buildTokenProofClaims(params, correlationId = "") {
		...
		if (params.nonce !== void 0) claims.nonce = params.nonce;
		return claims;
	}
	/**
	...
	*/
	async generateResourceProof(params, keyId, correlationId = "") {
		...
		if (!htu || !htm) throw createClientConfigurationError(dpopMissingResourceContext, correlationId);
...
function generateLibraryState(cryptoObj, correlationId, meta) {
	if (!cryptoObj) throw createClientAuthError(noCryptoObject, correlationId);
	...
	*/
	async handleServerTokenResponse(serverTokenResponse, authority, reqTimestamp, request, apiId, authCodePayload, userAssertionHash, handlingRefreshTokenResponse, forceCacheRefreshTokenResponse, serverRequestId, additionalCacheKeyComponents) {
		...
		if (request.authenticationScheme === AuthenticationScheme$1.DPOP) {
			if (serverTokenResponse.token_type?.toLowerCase() !== AuthenticationScheme$1.DPOP.toLowerCase()) {
				this.performanceClient?.addFields({ dpopTokenTypeMismatch: serverTokenResponse.token_type }, request.correlationId);
				throw createClientAuthError(dpopTokenTypeMismatch, request.correlationId);
			}
			serverTokenResponse.token_type = AuthenticationScheme$1.DPOP;
		}
		const attributeTokenPartition = serializeAttributeTokens(request.attributeTokens);
	...
	*/
	static async generateAuthenticationResult(cryptoObj, authority, cacheRecord, fromTokenCache, request, performanceClient, options = {}) {
		...
		if (cacheRecord.accessToken) {
			...
			if (accessTokenType === AuthenticationScheme$1.DPOP.toLowerCase()) {
				if (!cacheRecord.accessToken.keyId) throw createClientAuthError(keyIdMissing, request.correlationId);
				dpopProof = await new DpopProofGenerator(cryptoObj, tokenBindingKeyManager).generateResourceProof({
		...
		if (typeof regionSubScope === "string") performanceClient?.addFields({ regionSubScope }, request.correlationId);
		...
		return {
			...
			requestId: requestId || "",
			familyId,
			tokenType: cacheRecord.accessToken?.tokenType?.toLowerCase() === AuthenticationScheme$1.DPOP.toLowerCase() ? AuthenticationScheme$1.DPOP : cacheRecord.accessToken?.tokenType || "",
...
async function addDpopTokenProofHeader(headers, request, tokenEndpoint, cryptoUtils, tokenBindingKeyManager) {
	if (request.authenticationScheme !== AuthenticationScheme$1.DPOP) return;
	const dpopProofGenerator = new DpopProofGenerator(cryptoUtils, tokenBindingKeyManager);
	const keyId = request.dpopJkt;
	if (!keyId?.trim()) throw createClientAuthError(keyIdMissing, request.correlationId);
	headers[HeaderNames.DPOP] = await dpopProofGenerator.generateTokenProof({ tokenEndpoint }, keyId, request.correlationId);
}
/**
	...
	*/
	async createTokenRequestBody(request) {
		...
		if (request.authenticationScheme === AuthenticationScheme$1.POP) {
			...
			if (request.sshJwk) addSshJwk(parameters, request.sshJwk);
...
function getAuthorizationCodePayload(serverParams, cachedState, correlationId) {
	...
	if (!serverParams.code) throw createClientAuthError(authorizationCodeMissingFromServerResponse, correlationId);
	return serverParams;
}
/**
...
function validateAuthorizationResponse(serverResponse, requestState, correlationId) {
	if (!serverResponse.state || !requestState) throw serverResponse.state ? createClientAuthError(stateNotFound, correlationId, "Cached State") : createClientAuthError(stateNotFound, correlationId, "Server State");
	...
	if (decodedServerResponseState !== decodedRequestState) throw createClientAuthError(stateMismatch, correlationId);
	if (serverResponse.error || serverResponse.error_description || serverResponse.suberror) {
		const serverErrorNo = parseServerErrorNo(serverResponse);
		if (isInteractionRequiredError(serverResponse.error, serverResponse.error_description, serverResponse.suberror)) throw new InteractionRequiredAuthError(serverResponse.error || "", serverResponse.correlation_id || correlationId, serverResponse.error_description, serverResponse.suberror, serverResponse.timestamp || "", serverResponse.trace_id || "", serverResponse.claims || "", serverErrorNo);
...
var PerformanceClient = class {
	...
	*/
	removePerformanceCallback(callbackId) {
		...
		if (result) this.logger.verbose(`0253if ${callbackId}`, "");
		else this.logger.verbose(`0iqk07 ${callbackId}`, "");
		return result;
	}
	/**
...
var RefreshTokenClient = class {
	...
	*/
	async acquireTokenWithCachedRefreshToken(request, foci, apiId) {
		...
		if (refreshTokenRequest.authenticationScheme === AuthenticationScheme$1.DPOP && !refreshTokenRequest.dpopJkt) refreshTokenRequest.dpopJkt = await new DpopProofGenerator(this.cryptoUtils, this.config.tokenBindingKeyManager).generateJkt(request.correlationId);
		try {
...
var SilentFlowClient = class {
	...
	*/
	async acquireCachedToken(request) {
		...
		if (cachedAccessToken.tokenType?.toLowerCase() === AuthenticationScheme$1.DPOP.toLowerCase()) {
	...
	setCacheOutcome(cacheOutcome, correlationId) {
		...
		if (cacheOutcome !== CacheOutcome.NOT_APPLICABLE) this.logger.info(`09ingz ${cacheOutcome}`, correlationId);
	}
	/**
...
var UINT32_ARR = /* @__PURE__ */ new Uint32Array(1);
var RAW = "raw";
var ENCRYPT = "encrypt";
var DECRYPT = "decrypt";
...
var RSA_SIGN_ALGORITHM_OPTIONS = { name: PKCS1_V15_KEYGEN_ALG };
...
async function encrypt(baseKey, rawData, context) {
	...
	const nonce = window.crypto.getRandomValues(/* @__PURE__ */ new Uint8Array(16));
	const derivedKey = await deriveKey(baseKey, nonce, context);
	const encryptedData = await window.crypto.subtle.encrypt({
		name: AES_GCM,
		iv: /* @__PURE__ */ new Uint8Array(12)
...
function getJwkThumbprintMembers(publicJwk, correlationId) {
	...
	if (typeof kty !== "string" || kty.length === 0) throw createBrowserAuthError(invalidPublicJwk, correlationId, MISSING_JWK_KTY_SUBERROR);
	const requiredMembers = JWK_THUMBPRINT_REQUIRED_MEMBERS[kty];
	if (!requiredMembers) throw createBrowserAuthError(invalidPublicJwk, correlationId, UNSUPPORTED_JWK_KTY_SUBERROR);
	return requiredMembers.reduce((thumbprintMembers, memberName) => {
		const memberValue = publicJwk[memberName];
		if (typeof memberValue !== "string") throw createBrowserAuthError(invalidPublicJwk, correlationId, MISSING_JWK_MEMBER_SUBERROR);
		if (memberValue.length === 0) throw createBrowserAuthError(invalidPublicJwk, correlationId, EMPTY_JWK_MEMBER_SUBERROR);
...
async function computeJwkThumbprint(publicJwk, correlationId) {
	...
	return hashString(JSON.stringify(thumbprintMembers, Object.keys(thumbprintMembers).sort()));
}
//#endregion
...
async function waitForBridgeResponse(timeoutMs, logger, request, performanceClient, experimentalConfig) {
	return new Promise((resolve, reject) => {
		...
		channel.onmessage = (event) => {
			...
			else reject(createBrowserAuthError(redirectBridgeEmptyResponse, correlationId));
		};
	});
}
/**
...
function getHomepage(correlationId) {
	const urlComponents = new UrlString(window.location.href, correlationId || "").getUrlComponents();
...
var buildMergedClaims = buildMergedClaims$1;
//#endregion
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/crypto/TokenBindingKeyManager.mjs
/*! @azure/msal-browser v5.20.0 2026-08-28 */
/**
* Owns browser token-binding key lifecycle and storage lookup.
* @internal
*/
var TokenBindingKeyManager = class TokenBindingKeyManager {
	...
	static getTokenBindingKeyStorage(logger) {
		if (!TokenBindingKeyManager.tokenBindingKeyStorage) TokenBindingKeyManager.tokenBindingKeyStorage = new AsyncMemoryStorage(logger);
	...
	*/
	async removeTokenBindingKey(kid, correlationId) {
		...
		if (await this.cache.containsKey(kid, correlationId)) throw createClientAuthError(bindingKeyNotRemoved, correlationId);
	}
	/**
	...
	*/
	async clearKeystore(correlationId) {
		...
		try {
			...
			if (e instanceof Error) this.logger.error(`1owpn8 ${e.message}`, correlationId);
	...
	*/
	async getTokenBindingPublicKeyJwk(keyId, correlationId) {
		return exportJwk((await this.getTokenBindingKeyPair(keyId, correlationId)).publicKey);
	}
	...
	async generateKeyPairAndThumbprint(usages, keyGenAlgorithm, correlationId) {
		...
		const keyId = await computeJwkThumbprint(await exportJwk(keyPair.publicKey), correlationId);
		return {
	...
	async getTokenBindingKeyPair(keyId, correlationId) {
		...
		if (!cachedKeyPair) throw createBrowserAuthError(cryptoKeyNotFound, correlationId);
		return cachedKeyPair;
	}
	/** @internal */
	getTokenBindingKeyTelemetry(cachedKeyPair, fallbackAlgorithm) {
		return {
			...cachedKeyPair.tokenBindingKeyType && { tokenBindingKeyType: cachedKeyPair.tokenBindingKeyType },
			...cachedKeyPair.tokenBindingKeyAlgorithm || fallbackAlgorithm ? { tokenBindingKeyAlgorithm: cachedKeyPair.tokenBindingKeyAlgorithm || fallbackAlgorithm } : {}
		};
	}
	getTokenBindingKeyGenAlgorithmOptions(tokenBindingKeyAlgorithm, correlationId) {
		if (tokenBindingKeyAlgorithm === JsonWebTokenAlgorithms.RS256) return RSA_KEYGEN_ALGORITHM_OPTIONS;
		if (tokenBindingKeyAlgorithm === JsonWebTokenAlgorithms.ES256) return ECDSA_P256_KEYGEN_ALGORITHM_OPTIONS;
		throw createBrowserAuthError(unsupportedTokenBindingAlgorithm, correlationId);
	}
};
TokenBindingKeyManager.TOKEN_BINDING_KEY_USAGES = ["sign", "verify"];
//#endregion
...
var CryptoOps = class {
	...
	*/
	...
	getTokenBindingKeySigningAlgorithm(cachedKeyPair, requestedAlgorithm, correlationId) {
		...
		if (requestedAlgorithm === JsonWebTokenAlgorithms.RS256 && keyAlgorithm.name === RSA_SIGN_ALGORITHM_OPTIONS.name) return RSA_SIGN_ALGORITHM_OPTIONS;
		if (requestedAlgorithm === JsonWebTokenAlgorithms.ES256 && keyAlgorithm.name === ECDSA_SHA256_SIGN_ALGORITHM_OPTIONS.name && keyAlgorithm.namedCurve === ECDSA_P256_KEYGEN_ALGORITHM_OPTIONS.namedCurve) return ECDSA_SHA256_SIGN_ALGORITHM_OPTIONS;
		if (requestedAlgorithm === JsonWebTokenAlgorithms.RS256 || requestedAlgorithm === JsonWebTokenAlgorithms.ES256) throw createBrowserAuthError(unsupportedTokenBindingAlgorithm, correlationId, tokenBindingKeyAlgorithmMismatch);
		throw createBrowserAuthError(unsupportedTokenBindingAlgorithm, correlationId);
	}
	async validateTokenBindingJwtHeaderKey(header, kid, correlationId) {
		if (!header.jwk) return;
		if (await computeJwkThumbprint(header.jwk, correlationId) !== kid) throw createBrowserAuthError(invalidPublicJwk, correlationId, tokenBindingKeyJwkThumbprintMismatch);
	}
};
//#endregion
...
/*! @azure/msal-browser v5.20.0 2026-08-28 */
var COOKIE_LIFE_MULTIPLIER = 864e5;
var SameSiteOptions = {
	Lax: "Lax",
	None: "None"
};
...
var BrowserCacheManager = class extends CacheManager {
	...
	*/
	async migrateIdTokens(credentialSchema, accountSchema, correlationId) {
		...
		for (const idTokenKey of [...credentialKeysToMigrate.idToken]) {
			...
			if (!oldSchemaData) {
				switch (result.removalReason) {
					...
					case MigrationRemovalReason.DecryptFailed:
						...
						break;
					case MigrationRemovalReason.Invalid: this.performanceClient.incrementFields({ invalidITCount: 1 }, correlationId);
				}
			...
			const currentTokenHasSignInState = currentIdToken && Object.keys(extractTokenClaims(currentIdToken.secret, base64Decode, correlationId) || {}).includes("signin_state");
			/**
			* Only migrate if:
			...
			if (!oldSchemaData) {
				switch (result.removalReason) {
					...
					case MigrationRemovalReason.Expired:
						...
						break;
					case MigrationRemovalReason.Invalid: this.performanceClient.incrementFields({ invalidATCount: 1 }, correlationId);
				}
			...
			if (!oldSchemaData) {
				switch (result.removalReason) {
					...
					case MigrationRemovalReason.Expired:
						...
						break;
					case MigrationRemovalReason.Invalid: this.performanceClient.incrementFields({ invalidRTCount: 1 }, correlationId);
				}
	...
	*/
	trackVersionChanges(correlationId) {
		...
		if (previousVersion) {
			...
			this.performanceClient.addGlobalFields({ previousLibraryVersion: previousVersion });
		}
		if (previousVersion !== "5.20.0") this.setItem(VERSION_CACHE_KEY, version, correlationId);
	}
	/**
	...
	*/
	generateCredentialKey(credential, additionalCacheKeyHash) {
		...
		if (credential.additionalCacheKeyComponents && Object.keys(credential.additionalCacheKeyComponents).length > 0 && additionalCacheKeyHash) credentialKey.push(additionalCacheKeyHash);
	...
	*/
	async hydrateCache(result, request) {
		...
		*/
		const accessTokenEntity = createAccessTokenEntity(result.account.homeAccountId, result.account.environment, result.accessToken, this.clientId, result.tenantId, result.scopes.join(" "), result.expiresOn ? toSecondsFromDate(result.expiresOn) : 0, result.extExpiresOn ? toSecondsFromDate(result.extExpiresOn) : 0, base64Decode, request.correlationId || "", void 0, result.tokenType, void 0, request.sshKid);
...
var DEFAULT_BROWSER_CACHE_MANAGER = (clientId, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER) => {
	...
	}, DEFAULT_CRYPTO_IMPLEMENTATION, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager);
};
//#endregion
...
var EventHandler = class {
	constructor(logger) {
		...
		this.logger = logger || new Logger({}, "@azure/msal-browser", "5.20.0");
...
async function getDiscoveredAuthority(config, correlationId, performanceClient, browserStorage, logger, requestAuthority, requestAzureCloudOptions, requestExtraQueryParameters, account) {
	...
	if (account && !discoveredAuthority.isAlias(account.environment)) throw createClientConfigurationError(authorityMismatch, correlationId);
...
function validateSshRequest(request, correlationId) {
	if (!request.sshJwk) throw createClientConfigurationError(missingSshJwk, correlationId);
	if (!request.sshKid) throw createClientConfigurationError(missingSshKid, correlationId);
}
function validateDpopRequest(request, correlationId) {
	if (!request.resourceRequestMethod?.trim() || !request.resourceRequestUri?.trim()) throw createClientConfigurationError(dpopMissingResourceContext, correlationId);
}
/**
...
async function getTokenBindingRequestParams(request, tokenBindingKeyManager, logger, performanceClient) {
	switch (request.authenticationScheme) {
		case AuthenticationScheme$1.DPOP:
			if (request.platformBroker) return {};
			if (request.dpopJkt) return { dpopJkt: request.dpopJkt };
			return { dpopJkt: await tokenBindingKeyManager.provisionTokenBindingKey({
		...
		case AuthenticationScheme$1.POP:
			...
			if (request.popKid) return { reqCnf: cryptoOps.encodeKid(request.popKid) };
			const popTokenGenerator = new PopTokenGenerator(cryptoOps, tokenBindingKeyManager, performanceClient);
			return { reqCnf: (await invokeAsync(popTokenGenerator.generateCnf.bind(popTokenGenerator), PopTokenGenerateCnf, logger, performanceClient, request.correlationId)(request, logger)).reqCnfString };
		default: return {};
	}
}
/**
...
async function initializeBaseRequest(request, config, performanceClient, logger, correlationId) {
	...
	if (!validatedRequest.authenticationScheme) {
		...
		if (!SUPPORTED_AUTHENTICATION_SCHEMES.has(validatedRequest.authenticationScheme)) throw createClientConfigurationError(unsupportedAuthenticationScheme, correlationId);
		switch (validatedRequest.authenticationScheme) {
			case AuthenticationScheme$1.SSH:
				validateSshRequest(request, correlationId);
				break;
			case AuthenticationScheme$1.DPOP: validateDpopRequest(request, correlationId);
		}
...
function validateRequestMethod(interactionRequest, protocolMode) {
	...
	if (protocolMode === ProtocolMode.EAR) {
		if (requestMethod && requestMethod !== HttpMethod.POST) throw createClientConfigurationError(invalidRequestMethodForEAR, "");
...
/*! @azure/msal-browser v5.20.0 2026-08-28 */
//#endregion
//#region node_modules/@azure/msal-browser/dist/popup_relay/relayClient.mjs
/*! @azure/msal-browser v5.20.0 2026-08-28 */
/**
...
function buildPopupRelayUrl(popupRelayUri, id, action, correlationId) {
	...
	if (target.origin !== window.location.origin) throw createBrowserAuthError(popupRelayUnsupportedFlow, correlationId, "popup_relay_cross_origin");
...
async function waitForPopupRelayResponse(timeoutMs, logger, request, popupWindow, performanceClient) {
	return new Promise((resolve, reject) => {
		...
		const onMessage = (event) => {
			if (event.origin !== expectedOrigin || event.source !== popupWindow) return;
			const data = event.data;
			if (!data || data.type !== "msal:popup-relay-response:v1" || data.id !== expectedId) return;
			if (settled) return;
			settled = true;
			cleanup();
			if (data.error) reject(createBrowserAuthError(data.error, correlationId));
			else if (data.payload) resolve(data.payload);
			else reject(createBrowserAuthError(redirectBridgeEmptyResponse, correlationId));
		};
...
function extractBrowserRequestState(browserCrypto, state, correlationId) {
	...
	try {
		return parseRequestState(browserCrypto.base64Decode, state, correlationId).libraryState.meta;
	} catch (e) {
		throw createClientAuthError(invalidState, correlationId);
	}
}
...
function validateInteractionType(response, browserCrypto, interactionType, correlationId) {
	if (!response.state) throw createBrowserAuthError(noStateInHash, correlationId);
	const platformStateObj = extractBrowserRequestState(browserCrypto, response.state, correlationId);
	if (!platformStateObj) throw createBrowserAuthError(unableToParseState, correlationId);
	if (platformStateObj.interactionType !== interactionType) throw createBrowserAuthError(stateInteractionTypeMismatch, correlationId);
}
//#endregion
	...
	*/
	async handleCodeResponse(response, request, apiId) {
		...
		try {
			...
			if (e instanceof ServerError && e.subError === "user_cancelled") throw createBrowserAuthError(userCancelled, request.correlationId);
			else throw e;
		}
	...
	if (ext && ext.status) {
		switch (ext.status) {
			...
			case UI_NOT_ALLOWED:
				...
				break;
			default: error = new NativeAuthError(code, correlationId, description, ext);
		}
		return error;
	}
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
	...
	*/
	async acquireToken(request, cacheLookupPolicy) {
		...
		try {
			...
			nativeATMeasurement.add({ isNativeBroker: false });
			nativeATMeasurement.end({ success: false }, e);
			throw e;
		}
		try {
	...
	*/
	setBrokerErrorTelemetry(error) {
		if (error instanceof AuthError) this.performanceClient.addFields({
			isNativeBroker: true,
	...
	*/
	createSilentCacheRequest(request, cachedAccount) {
		...
		if (request.attributeTokens) silentRequest.attributeTokens = request.attributeTokens.split(" ");
	...
	*/
	async acquireTokensFromCache(nativeAccountId, request) {
		...
		if (!account) throw createClientAuthError(noAccountFound, this.correlationId);
		try {
	...
	*/
	async handleNativeResponse(response, request, reqTimestamp, storeInCache) {
		...
		else if (homeAccountIdentifier !== cachedhomeAccountId && response.account.id !== request.accountId) throw createNativeAuthError(userSwitch, this.correlationId);
		...
		await this.cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, result.tenantId, reqTimestamp, authority.getPreferredCache(), storeInCache);
		return result;
	}
	/**
	...
	*/
	async generatePopAccessToken(response, request) {
		if (request.tokenType === AuthenticationScheme$1.POP && request.signPopToken) {
			...
			*/
			if (!request.keyId) throw createClientAuthError(keyIdMissing, this.correlationId);
	...
	async generateAuthenticationResult(response, request, idTokenClaims, accountEntity, authority, reqTimestamp) {
		...
		*/
		if (accountInfo.nativeAccountId !== response.account.id) {
			...
			if (tenantProfile) tenantProfile.nativeAccountId = response.account.id;
		}
		...
		return {
			...
			...request.extraParameters?.resource && { resource: request.extraParameters.resource }
		};
	}
	/**
	...
	*/
	async cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId, reqTimestamp, environment, storeInCache) {
		...
		const additionalCacheKeyComponents = request.attributeTokens ? { attribute_tokens: request.attributeTokens } : void 0;
		const cachedAccessToken = createAccessTokenEntity(homeAccountIdentifier, environment, response.access_token, request.clientId, idTokenClaims.tid || tenantId, responseScopes.printScopes(), tokenExpirationSeconds, 0, base64Decode, request.correlationId, void 0, request.tokenType, void 0, request.keyId, additionalCacheKeyComponents);
		if (!!cachedIdToken && storeInCache?.idToken !== false) await this.browserStorage.setIdTokenCredential(cachedIdToken, this.correlationId, isKmsi(idTokenClaims));
	...
	*/
	async initializePlatformRequest(request) {
		...
		const mergedClaims = buildMergedClaims$1(claims, configClaims?.length ? configClaims : void 0);
		const hasAttributeTokens = !!request.attributeTokens?.length;
		this.performanceClient?.addFields({ hasAttributeTokens }, this.correlationId);
		const validatedRequest = {
			claims: mergedClaims,
		...
		if (hasAttributeTokens) validatedRequest.attributeTokens = serializeAttributeTokens(request.attributeTokens);
		if (validatedRequest.signPopToken && !!request.popKid) throw createBrowserAuthError(invalidPopTokenRequest, this.correlationId);
	...
	*/
	handleExtraBrokerParams(request, embeddedClientId) {
		...
		if (!embeddedClientId && !hasExtraBrokerParams) return;
		let child_client_id = "";
		...
		const { [BROKER_CLIENT_ID]: _brkClientId, [BROKER_REDIRECT_URI]: _brkRedirectUri, [CLIENT_ID]: _clientId, ...remainingExtraParameters } = request.extraParameters || {};
		request.extraParameters = {
...
/*! @azure/msal-browser v5.20.0 2026-08-28 */
var clientDataAccountTypeMapping = /* @__PURE__ */ new Map([["e", "AAD"], ["m", "MSA"]]);
/**
...
async function getStandardParameters(config, authority, request, logger, performanceClient) {
	...
	if (request.platformBroker) {
		...
		if (request.authenticationScheme === AuthenticationScheme$1.POP && request.reqCnf) addPopToken(parameters, request.reqCnf);
	}
	if (request.authenticationScheme === AuthenticationScheme$1.DPOP && request.dpopJkt) parameters.set(DPOP_JKT, request.dpopJkt);
	instrumentBrokerParams(parameters, request.correlationId, performanceClient);
	return parameters;
}
/**
...
async function getAuthCodeRequestUrl(config, authority, request, logger, performanceClient) {
	if (!request.codeChallenge) throw createClientConfigurationError(pkceParamsMissing, request.correlationId);
...
async function getEARFormData(config, authority, request, logger, performanceClient) {
	if (!request.earJwk) throw createBrowserAuthError(earJwkEmpty, request.correlationId);
	...
	return {
		action: getAuthorizeUrl(authority, queryParams),
		fields: Object.fromEntries(parameters)
	};
}
/**
...
async function handleResponsePlatformBroker(request, accountId, apiId, config, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
	...
	if (!platformAuthProvider) throw createBrowserAuthError(nativeConnectionNotEstablished, request.correlationId);
...
async function handleResponseEAR(request, response, apiId, config, authority, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
	...
	if (!response.ear_jwe) throw createBrowserAuthError(earJweEmpty, request.correlationId);
	if (!request.earJwk) throw createBrowserAuthError(earJwkEmpty, request.correlationId);
	...
	*/
	async sendPostRequestAsync(url, options) {
		...
		for (let attempt = 1; attempt <= 2; attempt++) try {
			...
			lastError = e;
			if (!shouldRetryPostFetchError(lastError, attempt)) throw createNetworkError(createBrowserAuthError(window.navigator.onLine !== false ? postRequestFailed : noNetworkConnectivity, correlationId || ""), void 0, void 0, lastError);
			await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY_MS));
			if (correlationId) performanceClient?.incrementFields({ fetchRetryCount: 1 }, correlationId);
		}
		if (!response) throw createNetworkError(createBrowserAuthError(window.navigator.onLine !== false ? postRequestFailed : noNetworkConnectivity, correlationId || ""), void 0, void 0, lastError);
		let responseStatus = 0;
		let responseHeaders = {};
		responseHeaders = getHeaderDict(response.headers);
		try {
...
function buildConfiguration({ auth: userInputAuth, cache: userInputCache, system: userInputSystem, experimental: userInputExperimental, telemetry: userInputTelemetry }, isBrowserEnvironment) {
	...
	if (userInputSystem?.protocolMode !== ProtocolMode.OIDC && userInputAuth?.OIDCOptions) new Logger(providedSystemOptions.loggerOptions, name, version).warning(JSON.stringify(createClientConfigurationError(cannotSetOIDCOptions, "")), "");
	...
	onChannelMessage(event) {
		...
		try {
			...
			if (method === NativeExtensionMethod.Response) {
				...
				if (response.status !== "Success") resolver.reject(createNativeAuthError(response.code, correlationId, response.description, response.ext));
				else if (response.result) {
					if (response.result["code"] && response.result["description"]) resolver.reject(createNativeAuthError(response.result["code"], correlationId, response.result["description"], response.result["ext"]));
					else resolver.resolve(response.result);
				} else throw createAuthError(unexpectedError, correlationId, "Event does not contain result.");
...
var PopupClient = class extends StandardInteractionClient {
	...
	async executeCodeFlow(request, popupParams, pkceCodes) {
		...
		try {
			...
			if (popupRequest.httpMethod === HttpMethod.POST) return await this.executeCodeFlowWithPost(popupRequest, popupParams, authClient, pkce.verifier);
				...
				const popupNavigateUrl = popupRelayUri ? buildPopupRelayUrl(popupRelayUri, this.getRelayStateId(request.state, correlationId), {
					method: "GET",
					url: navigateUrl
				}, correlationId) : navigateUrl;
	...
	*/
	async logoutPopupAsync(validRequest, popupParams, requestAuthority, mainWindowRedirectUri) {
		...
		try {
			...
			validRequest.state = setRequestState(this.browserCrypto, validRequest.state || "", { interactionType: InteractionType.Popup }, validRequest.correlationId);
			const logoutUri = authClient.getLogoutUri(validRequest);
			this.eventHandler.emitEvent(EventType.LOGOUT_SUCCESS, validRequest.correlationId, InteractionType.Popup, validRequest);
			const popupRelayUri = this.config.auth.popupRelayUri;
			const popupNavigateUrl = popupRelayUri ? buildPopupRelayUrl(popupRelayUri, this.getRelayStateId(validRequest.state || "", this.correlationId), {
				method: "GET",
				url: logoutUri
			}, this.correlationId) : logoutUri;
	...
	*/
	openPopup(urlNavigate, popupParams) {
		try {
			...
			if (!popupWindow) throw createBrowserAuthError(emptyWindowError, this.correlationId);
			try {
	...
	*/
	async openPostFormPopup(request, discoveredAuthority, popupParams, correlationId, isEAR) {
		const popupRelayUri = this.config.auth.popupRelayUri;
		if (popupRelayUri) {
			const formData = await (isEAR ? getEARFormData : getCodeFormData)(this.config, discoveredAuthority, request, this.logger, this.performanceClient);
		...
		(await (isEAR ? getEARForm : getCodeForm)(popupWindow.document, this.config, discoveredAuthority, request, this.logger, this.performanceClient)).submit();
		return popupWindow;
	}
	async waitForPopupResponse(request, popupWindow, popupWindowParent) {
		if (this.config.auth.popupRelayUri) return waitForPopupRelayResponse(this.config.system.popupBridgeTimeout, this.logger, request, popupWindow, this.performanceClient);
...
var RedirectClient = class extends StandardInteractionClient {
	...
	*/
	async acquireToken(request) {
		...
		try {
			...
			if (e instanceof AuthError) e.correlationId = this.correlationId;
	...
	*/
	getRedirectResponse(userProvidedResponse) {
		...
		if (!responseString) {
			if (this.config.auth.OIDCOptions.responseMode === ResponseMode$1.QUERY) responseString = window.location.search;
		...
		if (response) {
			try {
				...
				if (e instanceof AuthError) this.logger.error(`0bkq6p ${e.errorCode} ${e.errorMessage}`, this.correlationId);
				return [null, ""];
			}
			clearHash(window);
	...
	*/
	async handleResponse(serverParams, request, codeVerifier, serverTelemetryManager) {
		if (!serverParams.state) throw createBrowserAuthError(noStateInHash, request.correlationId);
	...
	*/
	async logout(logoutRequest) {
		...
		try {
			...
			validLogoutRequest.state = setRequestState(this.browserCrypto, validLogoutRequest.state || "", { interactionType: InteractionType.Redirect }, validLogoutRequest.correlationId);
			...
			const onRedirectNavigate = this.config.auth.onRedirectNavigate;
			if (typeof onRedirectNavigate === "function") {
				if (onRedirectNavigate(logoutUri) !== false) {
...
var SilentAuthCodeClient = class extends StandardInteractionClient {
	...
	*/
	async acquireToken(request) {
		if (!request.code) throw createBrowserAuthError(authCodeRequired, this.correlationId);
...
var StandardController = class StandardController {
	...
	*/
	verifySsoCapability(request, interactionType) {
		...
		const SSO_CAPABLE_TTL_MS = 864e5;
	...
	*/
	async acquireTokenByCode(request) {
		...
		try {
			if (request.code && request.nativeAccountId) throw createBrowserAuthError(spaCodeAndNativeAccountIdPresent, correlationId);
				...
				if (this.canUsePlatformBroker(request, request.nativeAccountId)) {
					...
					return result;
				} else throw createBrowserAuthError(unableToAcquireTokenFromNativePlatform, correlationId);
			} else throw createBrowserAuthError(authCodeOrNativeAccountIdRequired, correlationId);
		} catch (e) {
			this.eventHandler.emitEvent(EventType.ACQUIRE_TOKEN_FAILURE, correlationId, InteractionType.Silent, null, e);
	...
	*/
	async acquireTokenFromCache(commonRequest, cacheLookupPolicy) {
		switch (cacheLookupPolicy) {
			...
			default: throw createClientAuthError(tokenRefreshRequired, commonRequest.correlationId);
		}
	}
	/**
	...
	*/
	async hydrateCache(result, request) {
		...
		if (result.fromPlatformBroker) {
			...
			const attributeTokenPartition = serializeAttributeTokens(request.attributeTokens);
			const additionalCacheKeyComponents = attributeTokenPartition ? { attribute_tokens: attributeTokenPartition } : void 0;
			const accessTokenEntity = createAccessTokenEntity(result.account.homeAccountId, result.account.environment, result.accessToken, this.config.auth.clientId, result.tenantId, result.scopes.join(" "), result.expiresOn ? toSecondsFromDate(result.expiresOn) : 0, result.extExpiresOn ? toSecondsFromDate(result.extExpiresOn) : 0, base64Decode, request.correlationId || "", void 0, result.tokenType, void 0, request.sshKid, additionalCacheKeyComponents);
			if (request.resource) accessTokenEntity.resource = request.resource;
	...
	*/
	async acquireTokenNative(request, apiId, accountId, cacheLookupPolicy) {
		...
		if (!this.platformAuthProvider) throw createBrowserAuthError(nativeConnectionNotEstablished, correlationId);
	...
	*/
	async acquireTokenSilent(request) {
		...
		if (!account) throw createBrowserAuthError(noAccountError, correlationId);
		return this.acquireTokenSilentDeduped(request, account, correlationId).then((result) => {
			...
			if (error instanceof AuthError) error.correlationId = correlationId;
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/webBrokerBridge/WebBrokerBridgeError.mjs
/*! @azure/msal-browser v5.20.0 2026-08-28 */
/**
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/webBrokerBridge/WebBrokerBridgeErrorMap.mjs
/*! @azure/msal-browser v5.20.0 2026-08-28 */
/**
...
function toAuthError(err) {
	...
	switch (err.code) {
		case WebBrokerBridgeErrorCode.UserInteractionRequired: return new InteractionRequiredAuthError(err.innerErrorCode || "", correlationId, err.description, err.subError);
		case WebBrokerBridgeErrorCode.TransientError:
		case WebBrokerBridgeErrorCode.PersistentError: return new ServerError(err.innerErrorCode || "", correlationId, err.description, err.subError);
		case WebBrokerBridgeErrorCode.UserCanceled: return new ClientAuthError(userCanceled, correlationId);
		case WebBrokerBridgeErrorCode.NoNetwork: return new ClientAuthError(noNetworkConnectivity$1, correlationId);
		case WebBrokerBridgeErrorCode.AccountUnavailable: return new ClientAuthError(noAccountFound, correlationId);
		case WebBrokerBridgeErrorCode.BridgeDisabled: return new ClientAuthError(nestedAppAuthBridgeDisabled, correlationId);
		case WebBrokerBridgeErrorCode.BridgeUnavailable: return new ClientAuthError(err.innerErrorCode || "nested_app_auth_bridge_disabled", correlationId, err.description);
		...
		case WebBrokerBridgeErrorCode.ShrGenerationError: return new AuthError(err.code, correlationId, err.description, err.subError);
		case WebBrokerBridgeErrorCode.Unknown:
		default: return new AuthError(err.innerErrorCode || "unknown_error", correlationId, err.description || "An unknown error occurred", err.subError);
	}
}
//#endregion
...
var NestedAppAuthAdapter = class {
	...
	fromNaaTokenResponse(request, response, reqTimestamp) {
		if (!response.token.id_token || !response.token.access_token) throw createClientAuthError(nullOrEmptyToken, request.correlationId);
	...
	*/
	fromBridgeError(error) {
		if (isBridgeError(error)) return toAuthError(bridgeErrorToWebBrokerBridgeError(error));
	...
	*/
	toAuthenticationResultFromCache(account, idToken, accessToken, request, correlationId) {
		if (!idToken || !accessToken) throw createClientAuthError(nullOrEmptyToken, correlationId);
...
function bridgeErrorToWebBrokerBridgeError(error) {
	switch (error.status) {
		case BridgeStatusCode.UserCancel: return { code: WebBrokerBridgeErrorCode.UserCanceled };
		case BridgeStatusCode.NoNetwork: return { code: WebBrokerBridgeErrorCode.NoNetwork };
		case BridgeStatusCode.AccountUnavailable: return { code: WebBrokerBridgeErrorCode.AccountUnavailable };
		case BridgeStatusCode.Disabled: return { code: WebBrokerBridgeErrorCode.BridgeDisabled };
		case BridgeStatusCode.NestedAppAuthUnavailable: return {
			code: WebBrokerBridgeErrorCode.BridgeUnavailable,
			innerErrorCode: error.code,
			description: error.description
		};
		case BridgeStatusCode.TransientError: return {
			code: WebBrokerBridgeErrorCode.TransientError,
			innerErrorCode: error.code,
			description: error.description
		};
		case BridgeStatusCode.PersistentError: return {
			code: WebBrokerBridgeErrorCode.PersistentError,
			innerErrorCode: error.code,
			description: error.description
		};
		case BridgeStatusCode.UserInteractionRequired: return {
...
//#endregion
//#region node_modules/@azure/msal-browser/dist/webBrokerBridge/PendingRequestRegistry.mjs
/*! @azure/msal-browser v5.20.0 2026-08-28 */
/**
	...
	static async initializeNestedAppAuthBridge() {
		...
		try {
			window.nestedAppAuthBridge.addEventListener("message", (response) => {
				...
				if (responseEnvelope.success) BridgeProxy.pendingRegistry.resolve(responseEnvelope.requestId, responseEnvelope);
				else BridgeProxy.pendingRegistry.reject(responseEnvelope.requestId, responseEnvelope.error);
			});
...
async function loadExternalTokens(config, request, response, options, performanceClient = new StubPerformanceClient()) {
	...
	try {
		const idTokenClaims = response.id_token ? extractTokenClaims(response.id_token, base64Decode, correlationId) : void 0;
	...
	*/
	static getInteractionStatusFromEvent(message, currentStatus) {
		switch (message.eventType) {
			...
			case EventType.RESTORE_FROM_BFCACHE: if (message.interactionType === InteractionType.Redirect || message.interactionType === InteractionType.Popup) {
...
//#endregion
export { BrowserCacheLocation as A, InteractionRequiredAuthErrorCodes_exports as B, BrowserRootPerformanceEvents_exports as C, BrowserConfigurationAuthErrorCodes_exports as D, createGuid as E, BrowserAuthErrorCodes_exports as F, AuthenticationHeaderParser as G, LogLevel as H, BrowserAuthError as I, AuthErrorCodes_exports as J, ClientConfigurationErrorCodes_exports as K, enforceResourceParameter as L, InteractionStatus as M, InteractionType as N, BrowserConfigurationAuthError as O, WrapperSKU as P, AuthError as Q, ServerError as R, LocalStorage as S, BrowserUtils_exports as T, Logger as U, StubPerformanceClient as V, AzureCloudInstance as W, ClientAuthErrorCodes_exports as X, ProtocolMode as Y, ClientAuthError as Z, NavigationClient as _, ResponseMode as a, EventType as b, SignedHttpRequest as c, stubbedPublicClientApplication as d, PublicClientApplication as f, DEFAULT_IFRAME_TIMEOUT_MS as g, isPlatformBrokerAvailable as h, PromptValue as i, CacheLookupPolicy as j, ApiId as k, EventMessageUtils as l, createStandardPublicClientApplication as m, JsonWebTokenTypes as n, BrowserPerformanceMeasurement as o, createNestablePublicClientApplication as p, ClientConfigurationError as q, OIDC_DEFAULT_SCOPES as r, BrowserPerformanceClient as s, AuthenticationScheme as t, loadExternalTokens as u, EventHandler as v, MemoryStorage as w, SessionStorage as x, version as y, InteractionRequiredAuthError as z };
